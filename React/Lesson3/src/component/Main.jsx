...
class Main extends Component {
  render() {
    // T İÇİN this.props KISALTMASI object destructing
    const { t } = this.props
    return (
      <React.Fragment>
        ...
        <div className="alert alert-warning alert-dismissible fade show" role="alert">
          ...
          <strong className='text-danger'>{t('hello')}</strong>
          {/* INLINE CSS */}
          <span style={{ "color": "black" }}> SENSİNSS.</span>
        </div>


        {/* LİNK BUTTON FOR REACT DOM */}
        <Link to="http://localhost:3000/blog/list">
          <button className='btn btn-success mb-5 mt-5' >blog/list</button>
        </Link>
        <Link to="http://localhost:3000/blog/create">
          <button className='btn btn-success m-3' >blog/create</button>
        </Link>
